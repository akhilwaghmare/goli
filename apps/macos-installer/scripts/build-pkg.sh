...
(cd "$service_dir" && go build -ldflags "-X main.version=${VERSION:-0.1.0}" -o "$build_dir/root/usr/local/libexec/goli/goli" ./cmd/goli)
...
cp "$installer_dir/launchd/app.akhil.goli.plist" "$build_dir/root/Library/LaunchDaemons/app.akhil.goli.plist"
...
pkgbuild --root "$build_dir/root" --scripts "$build_dir/scripts" --identifier app.akhil.goli --version "${VERSION:-0.1.0}" --install-location / --filter '(^|/)\._.*$' --filter '(^|/)\.DS_Store$' --filter '(^|/)(\.svn|CVS)(/|$)' "$build_dir/goli-component.pkg"
